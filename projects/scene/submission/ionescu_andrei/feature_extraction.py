...
class SIFTFeatureTransformer(FeatureTransformer):
  def __init__(self, visualize: bool = False,
    ...
    self._transformer: cv2.SIFT = cv2.SIFT_create()
...
class ColorHistFeatureExtractor(FeatureExtractor):
  ...
  def __str__(self) -> str:
    return f'FE_ColorHist (intervals={self._intervals}, density={self._density}, range={self._range})'
...
class BOVWFeatureExtractor(FeatureExtractor):
  ...
  def __str__(self) -> str:
    return f'FE_BOVW_{self._transformer}'

  def __repr__(self) -> str:
    return f'FE_BOVW_{self._transformer} (n_features={self._n_features})'


class ImageFeatureExtractor(FeatureExtractor):
  def __init__(self, grayscale: bool = False, min_max: bool = True) -> None:
    super().__init__(grayscale, min_max)

    # Internal params
    self._std: ny.ndarray = None
    self._mean: ny.ndarray = None

  def fit_transform(self, data: ny.ndarray) -> None:
    data = self.preprocess(data)
    self._std: ny.ndarray = ny.std(data, axis=(0, 2, 3)).reshape((1, data.shape[1], 1, 1))
    self._mean: ny.ndarray = ny.mean(data, axis=(0, 2, 3)).reshape((1, data.shape[1], 1, 1))

  def transform(self, data: ny.ndarray) -> ny.ndarray:
    data = self.preprocess(data)
    return ((data - self._mean) / self._std).reshape((data.shape[0], -1))

  def transform_image(self, image: ny.ndarray) -> ny.ndarray:
    image = self.preprocess(image[None, ...]).squeeze(0)
    return ((image[None, ...] - self._mean) / self._std).flatten()

  def __str__(self) -> str:
    return f'FE_ImgNorm'

  def __repr__(self) -> str:
    return f'FE_ImgNorm (grayscale={self._grayscale}, min_max={self._min_max})'
